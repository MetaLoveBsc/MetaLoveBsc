...
// will be transferred to a governance smart contract once MLC is sufficiently
...
contract MasterChefMLC is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of MLCs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accMLCPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accMLCPerShare` (and `lastRewardBlock`) gets updated.
    ...
    constructor(
        ...
        require(_mlcPerBlock <= MAX_EMISSION_RATE, "Tokens per block more than allowed");
    ...
    // View function to see pending MLCs on frontend.
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 mlcReward = multiplier.mul(mlcPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        mlc.mint(feeAddress, mlcReward.mul(3).div(100));
    ...
    // Pay or lockup pending MLC's.
    function payOrLockupPendingCryptoMLC(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        if (canHarvest(_pid, msg.sender)) {
            if (pending > 0 || user.rewardLockedUp > 0) {
                ...
                safeMLCTransfer(msg.sender, totalRewards);
    ...
    // Safe mlc transfer function, just in case if rounding error causes pool to not have enough MLCs.
